...
export default class EntityLiving implements IEntity<Humanoid, BasePart, HumanoidDescription>, IEntityLiving {
	Character: Model | Actor;
	Pitch: number;
	Sneaking: boolean;
	Sprinting: boolean;
	...
	Age: number;
	Attacking: boolean;
	AttackTarget?: ICustomNPC | Player | undefined;
	LastAttackedTime?: number | undefined;
	LastAttacked?: ICustomNPC | Player | undefined;
	MainHandItem?: MeshPart | BasePart | Model | undefined;
	OffHandITem?: MeshPart | BasePart | Model | undefined;

	SeenEntities?: (ICustomNPC | Player | undefined)[];

	constructor(Info: EntityLivingProperties) {
		this.Attacking = Info?.Attacking || false;
		this.AttackTarget = Info?.AttackingTarget || undefined;
		this.LastAttacked = Info?.LastAttacked || undefined;
		this.LastAttackedTime = undefined;
		this.MainHandItem = Info?.MainHandItem || undefined;
		this.OffHandITem = Info?.OffHandITem || undefined;
	...
	canSeeEntity(entity: ICustomNPC | Player | undefined): boolean {
		// this.SeenEntities.forEach((element) => {
		// 	if (element === entity) {
		// 		return true;
		// 	}
		// });
	...
	getLastAttackedTime(): number {
		return this.LastAttackedTime || 0;
	}
	/**
